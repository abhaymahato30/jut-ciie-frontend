...
export default function Navbar() {
  ...
  className="mt-2 mb-2 rounded-lg bg-accent px-4 py-2 text-center font-semibold text-white transition hover:opacity-90"
>
  Apply Now
</a>
          ...
          <div className="flex flex-col px-4 py-4">
            {navLinks.map((link) => (
              <NavLink
                ...
                onClick={() => setIsOpen(false)}
                className={({ isActive }) =>
                  `border-b border-gray-100 py-3 transition-all duration-300 ${
                    isActive
                      ? "font-semibold text-[#78B3A8]"
                      : "text-primary hover:text-[#78B3A8]"
                  }`
                }
              >
                {link.label}
